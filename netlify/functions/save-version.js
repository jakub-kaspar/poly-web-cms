...
exports.handler = async (event) => {
  ...
  try {
    const { id, state, offerVersion, changes } = JSON.parse(event.body);
    if (!id || !state) return { statusCode: 400, body: JSON.stringify({ error: "Missing id or state" }) };

    const metaFile = await ghGet(`offers/${id}.json`);
    ...
    await ghPut(`offers/${id}.json`, JSON.stringify(meta, null, 2), commitMsg, metaFile.sha);

    const stateFile = await ghGet(`offers/${id}-state.json`);
    await ghPut(`offers/${id}-state.json`, JSON.stringify({ version: offerVersion, state }, null, 2), commitMsg, stateFile.sha);
